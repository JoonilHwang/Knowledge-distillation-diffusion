...
def Tensor2numpy(batch):
    fn_tonumpy = lambda x: x.to('cpu').detach().numpy().transpose(0, 2, 3, 1)
    batch = fn_tonumpy(batch)
    return batch
def save_single_image_tiff(image, save_path, file_name):
    image = image.detach().clone()
    image = image.permute(1, 2, 0).to('cpu', torch.float32).numpy()
    #print(image.shape)
    #im = Image.fromarray(image)
    #im.save(os.path.join(save_path, file_name))   
    io.imsave(os.path.join(save_path, file_name),image[:,:,0]) 
@torch.no_grad()
