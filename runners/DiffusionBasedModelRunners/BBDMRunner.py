...
from runners.utils import weights_init, get_optimizer, get_dataset, make_dir, get_image_grid, save_single_image, Tensor2numpy, save_single_image_tiff
...
class BBDMRunner(DiffusionBaseRunner):
    ...
    def sample(self, net, batch, sample_path, stage='train'):
        ...
        sample_cpu = Tensor2numpy(sample)
        io.imsave(os.path.join(sample_path, 'sample.tiff'),sample_cpu[0])
        ...
        x_cond_cpu = Tensor2numpy(x_cond)
        io.imsave(os.path.join(sample_path, 'condition.tiff'),x_cond_cpu[0,:,:,0])
        ...
        x_cpu = Tensor2numpy(x)
        io.imsave(os.path.join(sample_path, 'ground_truth.tiff'),x_cpu[0,:,:,0])
            ...
            for j in range(sample_num):
                ...
                for i in range(batch_size):
                    ...
                    if j == 0:
                        ...
                        save_single_image_tiff(condition, condition_path, f'{x_cond_name[i]}.tiff')
                        save_single_image_tiff(gt, gt_path, f'{x_name[i]}.tiff')
                    if sample_num > 1:
                        ...
                        save_single_image_tiff(result, result_path_i, f'output_{j}.tiff')
                        #save_single_image_tiff(gt, gt_path, f'output_{j}.tiff')

                    else:
                        save_single_image(result, result_path, f'{x_name[i]}.png', to_normal=to_normal)
                        save_single_image_tiff(result, result_path, f'{x_name[i]}.tiff')
